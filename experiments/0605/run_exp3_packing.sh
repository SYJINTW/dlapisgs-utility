#!/usr/bin/env bash
# Experiment 3 — packing comparison: nocull floor / progressive / tile_strict ceiling.
# Fixed:  weight_mode=screen_area, img=1600x1600.
# Three conditions:
#   nocull      — grid=1x1x1, progressive, vd_lod        (all 10 scenes)
#   progressive — grid=8x8x8, progressive, vd_lod        (all 10 scenes)
#   tile_strict — grid=8x8x8, tile_strict, vd_lod+oracle_loo+ml  (all 10 scenes; skip if oracle/ML missing)
# Oracle NPZs: output/0527_clean/exp4_oracle_dq/eval/{scene}/oracle_dq.npz
# ML models:   ml/models_clean/{scene}/AC/
#
# Env overrides:
#   SCENES="chair bicycle"
#   CUDA_VISIBLE_DEVICES=0
#   OUTPUT_ROOT=.../output/MMDD/exp3_packing
set -euo pipefail
...
NUM_LOD="${NUM_LOD:-1}"
OUT_BASE="${OUTPUT_ROOT:-$UTIL_DIR/output/0605/exp3_packing}"

echo "=========================================="
echo "Exp 3 @1600² — nocull / progressive / tile_strict"
...
# ── No-cull baseline: 1x1x1 progressive (all scenes) ─────────────────────────
echo ""
echo "=== NOCULL (1x1x1 progressive, vd_lod) ==="
for scene in $SCENES; do
    PLY="$DSET/$scene/point_cloud.ply"
    TRACE="$DSET/$scene/sparse_views_eval.json"
    GT_DIR="$DSET/$scene/gt_renders_eval"
    OUT_DIR="$OUT_BASE/$scene/nocull"
...
# ── Progressive: 8x8x8 with FOV culling (all scenes) ─────────────────────────
echo ""
echo "=== PROGRESSIVE (8x8x8, vd_lod) ==="
for scene in $SCENES; do
    PLY="$DSET/$scene/point_cloud.ply"
    TRACE="$DSET/$scene/sparse_views_eval.json"
    GT_DIR="$DSET/$scene/gt_renders_eval"
    TILING_CACHE="$OUT_BASE/$scene/.tiling_cache.npz"
    OUT_DIR="$OUT_BASE/$scene/progressive"

    if [[ ! -f "$PLY" ]];    then echo "[skip] $scene: no point_cloud.ply";       continue; fi
    if [[ ! -f "$TRACE" ]];  then echo "[skip] $scene: no sparse_views_eval.json"; continue; fi
    if [[ ! -d "$GT_DIR" ]]; then echo "[skip] $scene: no gt_renders_eval/";       continue; fi

    echo "---- [$scene] progressive ----"
    mkdir -p "$OUT_DIR" "$(dirname "$TILING_CACHE")"
    check_gpu_free
...
# ── Tile-strict: oracle+ML ceiling (all scenes; skip if oracle/ML missing) ───
echo ""
echo "=== TILE_STRICT (8x8x8, vd_lod+oracle_loo+ml) ==="
for scene in $SCENES; do
    PLY="$DSET/$scene/point_cloud.ply"
    TRACE="$DSET/$scene/sparse_views_eval.json"
    GT_DIR="$DSET/$scene/gt_renders_eval"
    ...
    TILING_CACHE="$OUT_BASE/$scene/.tiling_cache.npz"
    OUT_DIR="$OUT_BASE/$scene/tile_strict"
...
for scene in $SCENES; do
    ...
    [[ -f "$SCENE_CSV" ]] && conda run -n gsquic python "$UTIL_DIR/experiments/plot_metrics.py" \
        ...
        --group-by packing_mode \
        --title-suffix "$scene (Exp3 @1600² screen_area)" || true
done
