#!/usr/bin/env bash
# Exp: w-mode mean ablation.
# Question: does W_k = mean(w_gi) = Σw/N_k beat the default W_k = Σw(sum)?
# Follows exp2 structure (tile_strict, 4 schemes, 7 budgets).
#
# Env overrides:
#   SCENES="hotdog ship"
...
SCHEMES_LIST="${SCHEMES_LIST:-vd_lod vd_lod_w}"
GRID_SHAPE="${GRID_SHAPE:-8 8 8}"
NUM_LOD="${NUM_LOD:-1}"
CAMERA_INDEX="${CAMERA_INDEX:--1}"
W_MODE="mean"
RUN_DATE="${RUN_DATE:-0526}"
OUT_BASE="${OUTPUT_ROOT:-$UTIL_DIR/output/$RUN_DATE/exp_wmode_mean}"
...
echo "=========================================="
echo "Exp w-mode=mean ablation — output_root=$OUT_BASE"
echo "=========================================="
...
for scene in $SCENES; do
    ...
    for wm in $WEIGHT_MODES; do
        ...
        echo ""
        echo "---- [$scene] weight_mode=$wm w_mode=$W_MODE  schemes=[$SCHEMES_LIST]  budget_pcts=[$BUDGET_PCTS] ----"
        ...
        conda run -n "$CONDA_ENV" python "$UTIL_DIR/experiments/plot_metrics.py" \
            ...
            --group-by scheme \
            --title-suffix "$scene (w-mode=mean, w=$wm)"
